function L06E01_trapezoidalRule1
...
% % f(b) - f(a) = A*b + B - (A*a + B)
% % f(b) - f(a) = A*b + B - A*a - B
% % f(b) - f(a) = A*b - A*a + B - B % re-arranging
% % f(b) - f(a) = A*(b - a)
% % or, A = (f(b) - f(a)) / (b - a)
% % 
...
% % the width between the start and end points along the x-axis
% % i.e. - (b - a)
% % 
% % multiplied by
% % 
% % the average of the function heights at the start and end points
% % (i.e. - (f(a) + f(b)) /2
% % 
trapArea1 = (xMax - xMin) * (f(xMin) + f(xMax) / 2);
...
% % APPROACH 3: Using MATLAB's inbuilt trapezoid area function
trapzArea = trapz(x,0.2 + 25.*x - 200.*x.^2 + 675.*x.^3 - 900.*x.^4 + 400.*x.^5);

% % APPROACH 4: Calculate the area under the curve via integration
integratedArea = integral(@(x)f(x),xMin,xMax);

% % The errors between Approaches 1 & 2 versus 3 & 4
err_trapz1 = 100*abs((trapzArea - trapArea1) / trapzArea);
err_trapz2 = 100*abs((trapzArea - trapArea2) / trapzArea);
err_int1 = 100*abs((integratedArea - trapArea1) / integratedArea);
err_int2 = 100*abs((integratedArea - trapArea2) / integratedArea);
...
disp(['Scripting using Approach 1                    = ',num2str(trapArea1)]);
disp(['Scripting using Approach 2                    = ',num2str(trapArea2)]);
disp(['MATLAB''s in-built "trapz" function            = ',num2str(trapzArea)]);
disp(['MATLAB''s integrated area function             = ',num2str(integratedArea)]);
disp('----')
disp(['error between Approach 1 and "trapz"     (%)  = ',num2str(err_trapz1)]);
disp(['error between Approach 2 and "trapz"     (%)  = ',num2str(err_trapz2)]);
disp('----')
disp(['error between Approach 1 and integration (%)  = ',num2str(err_int1)]);
disp(['error between Approach 2 and integration (%)  = ',num2str(err_int2)]);
disp('----')
