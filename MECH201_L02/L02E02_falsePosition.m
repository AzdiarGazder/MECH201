...
if (fxL*fxU >= 0)
    ...
    disp('The root cannot be found using the false position method. Please use another method.');
    return;
else

    if fxL < 0
        temp_xU = xU; temp_fxU = fxU;
        xU = xL;      fxU = fxL;
        xL = temp_xU; fxL = temp_fxU;
    end
...
if solNFound % if solution found
    ...
    disp('-------')
    disp('Solution found.')
    disp(['False position converged in ',num2str(iter), ' iteration(s).'])
    disp(['Root = ',num2str(x0)]);
    disp(['Function value at root = ',num2str(fx0)]);
    disp('-------')

else % if no solution found
    error('No solution. False position did not converge in %d iteration(s).', maxIter);
end
toc

error = arrayfun(@(x) str2double(x), string(T.err));  % convert table columns to double array
plot(1:length(error),error,'o-k','LineWidth',2,'DisplayName',['falsePosition']);
hold all;
xlabel('iteration (s)')
ylabel('error (abs)')
