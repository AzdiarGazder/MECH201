function L08E02_rungeKutta_heunMethod
...
%     Y(1) = 2;                    % starting value of Y @ xMin (known)
    ...
    tic
    for jj = 1:n
        fValue_1 = f(X(jj), Y(jj)); % f(x,y) using current X & Y values
        fValue_2 = f(X(jj + 1), (Y(jj) + (fValue_1 * stepSize(ii)))); % f(x,y) using (X = succeeding X value) and (Y = current Y value + (current f(x,y) * stepSize))
        
        Y(jj + 1) = Y(jj) + (((fValue_1 + fValue_2) / 2) * stepSize(ii)); % final succeeding Y value
    end
