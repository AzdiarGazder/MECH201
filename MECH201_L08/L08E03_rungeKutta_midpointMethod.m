function L08E03_rungeKutta_midpointMethod
...
for ii = 1:length(stepSize)
    ...
    tic
    for jj = 1:n
        fValue = f(X(jj), Y(jj)); % f(x,y) using current X & Y values
        
        Xmid = X(jj) + (stepSize(ii)/2); % mid X value
        Ymid = Y(jj) + (fValue * (stepSize(ii)/2)); % mid Y value  = current Y value + (current f(x,y) * stepSize/2)
        
        Y(jj + 1) = Y(jj) + (f(Xmid, Ymid) * stepSize(ii)); % succeeding Y value
    end
