function L08E08_simultaneousODEs_rungeKutta_midpointMethod
...
for jj = 1:n
    disp('----')
    fValue = f(X(jj), Y(:,jj)) % f(x,y) using current X and Y1 & Y2 values

    Xmid = X(jj) + (stepSize/2); % mid X value
    Ymid(:,jj) = Y(:,jj) + (fValue * (stepSize/2)); % mid Y1 & Y2 values  = current Y1 & Y2 values + (current f(x,y) * stepSize/2)

    Y(:, jj + 1) = Y(:,jj) + (f(Xmid, Ymid) * stepSize) % succeeding Y1 & Y2 values
end
%%
