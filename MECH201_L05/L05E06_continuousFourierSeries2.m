function L05E06_continuousFourierSeries2
...
for k = 1:50
    ak = @(x) (1/Tf) * integral(@(x)F(x).*cos(k*x),Ts,Tf) * cos(k*x);
    bk = @(x) (1/Tf) * integral(@(x)F(x).*sin(k*x),Ts,Tf) * sin(k*x);
    sumEq = @(x) sumEq(x) + (ak(x) + bk(x)); % do the summation
end
