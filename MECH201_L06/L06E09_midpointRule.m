...
x = linspace(xMin,xMax,n+1);  % define **EQUALLY** spaced x-values
                              % between the bounds
stepSizeX = (xMax - xMin) / n; % the width is the same for **EQUALLY**
                              % spaced x-values between the bounds

midpointArea = 0;
for ii = 1:n
    midPointX(ii) = (x(ii) + (x(ii) + stepSizeX)) / 2; % generate the list of midpoints
    midpointArea = midpointArea + (stepSizeX * f(midPointX(ii))); % sum the area of the midpoints
end
